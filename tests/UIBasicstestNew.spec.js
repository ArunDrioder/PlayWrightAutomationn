...
//     const userName = page.getByPlaceholder('email@example.com'); // This will create a locator for the input field with the placeholder 'Username'. You can use this locator to interact with the element later in your test.
//     const password = page.getByPlaceholder('enter your passsword'); // This will create a locator for the input field with the placeholder 'Password'. You can use this locator to interact with the element later in your test.
//     const signInButton = page.getByRole('button',{name:"Login"}); // This will create a locator for the input field with the type 'submit'. You can use this locator to interact with the element later in your test.
...
test.only ('End-to-End Test', async ({ browser }) => {
    ...
    //console.log(allProductTitles);
     ...
     await page.locator(".card-body").filter({hasText:"ZARA COAT 3"}).getByRole("button",{name:" Add To Cart"}).click(); // This will click the button with the name 'Add To Cart' within the element with the class 'card-body' that contains the text 'ZARA COAT 3'. You can use this locator to interact with the add to cart button later in your test, such as checking its visibility or clicking on it.
    
     await page.getByRole("listitem").getByRole('button',{name:"Cart"}).click(); // This will click the button with the name 'Cart', which is likely a button to navigate to the cart page. You can use this locator to interact with the cart button later in your test, such as checking its visibility or clicking on it.
     await page.locator("div li").first().waitFor();
     await expect(page.getByText("ZARA COAT 3")).toBeVisible(); // This will wait until the element containing the text 'ZARA COAT 3' is attached to the DOM and has a non-empty text content. This is useful to ensure that the cart page has fully loaded and the product is ready for interaction before proceeding with the test.
     await page.getByRole("button", { name: "Checkout" }).click();
     await page.getByPlaceholder("Select Country").pressSequentially("Ind", { delay: 150 });
     await page.getByRole("button", { name: "India" }).nth(1).click();
     await page.getByText("PLACE ORDER").click();
     await expect(page.getByText("Thankyou for the order.")).toBeVisible(); // This will wait until the element containing the text 'Thank you for the order.' is attached to the DOM and has a non-empty text content. This is useful to ensure that the order confirmation page has fully loaded and the confirmation message is ready for interaction before proceeding with the test.
});
